...
APR_DECLARE(apr_status_t) stomp_read(stomp_connection *connection, stomp_frame **frame, apr_pool_t *pool) {
   ...
   if( f->headers == NULL )
      return APR_ENOMEM;
   apr_hash_set(f->headers, "destination", APR_HASH_KEY_STRING, "/queue/FOO.BAR");
   ...
   // Parse the frame out.
   {
      ...
      // Check for content length
	  {
		  ...
		  if(content_length) {
			  ...
			  apr_size_t body_length = f->body_length;

			  rc = apr_socket_recv(connection->socket, f->body, &body_length);
			  CHECK_SUCCESS;

			  // Dont !! Expect a \n after the end
			  rc = apr_socket_recv(connection->socket, endbuffer, &length);
			  CHECK_SUCCESS;
			  if(length != 1 || endbuffer[0] != '\0' )
				  return APR_EGENERAL;
		  }
		  else
		  {
